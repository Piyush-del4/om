  ...
  return (
    <div className="relative radial-mesh-bg min-h-screen bg-black overflow-hidden py-24 px-4 sm:px-6 lg:px-8 text-white">
      <div className="max-w-2xl mx-auto space-y-12 relative z-10">
        ...
        {/* Avatar */}
        <div className="flex items-center gap-6 p-4 bg-neutral-900/40 border border-neutral-800/60 rounded-2xl transition-spring">
          ...
          <div className="space-y-1">
            <p className="font-serif text-xl font-bold text-white">{user.name}</p>
        ...
        <GoldCard theme="dark" className="transition-spring">
          <div className="space-y-6">
            <h2 className="font-serif text-xl font-bold flex items-center gap-2.5 pb-2 border-b border-neutral-800/60 text-[var(--gold)]">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-6">
              <div className="space-y-1">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full input-underline text-white text-sm py-2" />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 9999999999" className="w-full input-underline text-white text-sm py-2" />
              </div>
              <div className="pt-2">
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={handleChangePassword} className="space-y-6">
              <div className="space-y-1">
                ...
                <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} required placeholder="••••••••" className="w-full input-underline text-white text-sm py-2" />
              </div>
              <div className="space-y-1">
                <label className="block text-[10px] font-semibold uppercase tracking-widest text-[var(--gold-light)]">New Password</label>
                <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required placeholder="••••••••" className="w-full input-underline text-white text-sm py-2" />
              </div>
              <div className="space-y-1">
                <label className="block text-[10px] font-semibold uppercase tracking-widest text-[var(--gold-light)]">Confirm New Password</label>
                <input type="password" value={confirmNewPassword} onChange={(e) => setConfirmNewPassword(e.target.value)} required placeholder="••••••••" className="w-full input-underline text-white text-sm py-2" />
              </div>
              <div className="pt-2">
