  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-gray-400 px-4 text-center">
        ...
        <h2 className="font-serif text-2xl font-bold text-white mb-2">No Items For Checkout</h2>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-7 space-y-6">
        <GoldCard theme="dark" className="border border-[var(--gold-200)]/30 p-6 sm:p-8">
      ...
      <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
        <GoldCard theme="dark" className="border border-[var(--gold-100)] p-6 space-y-6">
            ...
            {items.map((item: any) => {
              ...
              return (
                <div key={product._id} className="flex gap-4 items-center">
                  {product.imageUrl && (
                    <div className="w-12 h-12 rounded bg-neutral-900 overflow-hidden flex-shrink-0 border border-neutral-800/80">
                      <img src={product.imageUrl} alt={product.title} className="w-full h-full object-cover" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-white truncate">{product.title}</h4>
                    <p className="text-gray-400 text-[10px] font-mono">Qty: {item.quantity}</p>
                  </div>
          ...
          <div className="space-y-2 text-xs font-mono">
            ...
            <div className="h-px bg-neutral-900/60 my-2"></div>
    ...
    <div className="relative radial-mesh-bg min-h-screen bg-black overflow-hidden py-24 px-4 sm:px-6 lg:px-8 text-white">
      <div className="max-w-6xl mx-auto space-y-8 relative z-10 animate-fade-in">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-[var(--gold-200)]">
          <button onClick={() => window.history.back()} className="group flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-gray-400 hover:text-[var(--gold)] transition-colors">
