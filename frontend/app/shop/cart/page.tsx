          ...
          <>
            <div className="space-y-4">
              {cartItems.map((item: any) => {
                ...
                return (
                  <GoldCard key={product._id} theme="dark" className="border border-[var(--gold-100)] p-4">
                    <div className="flex items-center gap-4">
                      {product.imageUrl && (
                        <div className="w-16 h-16 rounded-lg overflow-hidden bg-neutral-900 flex-shrink-0 relative">
                          ...
                          {isOutOfStock && (
                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                              <span className="text-[8px] bg-red-600 px-1 py-0.5 rounded text-white font-mono font-bold">SOLD OUT</span>
                            </div>
                          )}
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-sm text-white truncate">{product.title}</h3>
                          {isOutOfStock && (
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-7 h-7 flex items-center justify-center border border-neutral-700 disabled:opacity-30 rounded-md text-gray-300 hover:border-[var(--gold)] hover:text-[var(--gold)] transition-colors"
                        >
                      ...
                      <p className="text-white font-bold text-sm w-20 text-right">₹{(isOutOfStock ? 0 : (product.price * item.quantity) / 100).toLocaleString()}</p>
            ...
            <GoldCard theme="dark" className="border border-[var(--gold-200)] p-6 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-gray-300"><span>Subtotal</span><span>₹{(subtotal / 100).toLocaleString()}</span></div>
                <div className="flex justify-between text-gray-300"><span>GST (18%)</span><span>₹{(gst / 100).toLocaleString()}</span></div>
                <div className="h-px bg-neutral-800"></div>
                <div className="flex justify-between text-white font-bold text-base"><span>Total</span><span className="text-[var(--gold)]">₹{(total / 100).toLocaleString()}</span></div>
              </div>
