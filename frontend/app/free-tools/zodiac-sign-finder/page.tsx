...
import { Compass, Search, ArrowRight, HelpCircle } from 'lucide-react';
...
const FAQS = [
 ...
 { q: 'How accurate is this Zodiac Sign Finder?', a: 'Our calculator uses precise astronomical ephemeris data to generate highly accurate results based on your exact birth time.' },
 { q: 'Is this service completely free?', a: 'Yes, this tool is 100% free to use for unlimited calculations.' },
 { q: 'Do I need my exact birth time?', a: 'While the Sun stays in a sign for about 30 days, exact birth time is required if you were born on a cusp (the day the sign changes).' },
...
export default function ZodiacSignFinderPage() {
 ...
 const sunData = Object.values(planetsObj).find((p: any) => p.name === 'Sun');
 
 if (data.success && sunData) {
 setResultData(sunData);
 setResult(true);
 } else {
 ...
 <Compass className="w-4 h-4" /> Free Utility
 </span>
 <h1 className="font-serif text-4xl md:text-5xl font-bold">
 Finder <span className="gold-gradient-text">Zodiac Sign</span>
 </h1>
 <p className="text-gray-600 text-sm md:text-base font-light max-w-2xl mx-auto">
 Find your true Sun and Moon signs based on birth details. Enter your details below to calculate your personalized report instantly.
 </p>
 </motion.div>
 ...
 Here is the precise Zodiac (Sun) Sign for <span className="capitalize text-[var(--gold)]">{formData.name}</span>.
 </p>
 <div className="bg-gray-100 border border-gray-200 rounded-xl p-8 text-center max-w-sm mx-auto">
 <div className="text-[var(--gold)] font-serif text-5xl font-bold mb-4 drop-shadow-[0_0_15px_rgba(234,179,8,0.3)]">
 {ZODIAC_SIGNS[resultData?.current_sign - 1] || 'Unknown'}
 </div>
 <div className="text-sm text-gray-600 font-light">
 Your Sun is positioned at {resultData?.normDegree?.toFixed(2)}° in the sign of {ZODIAC_SIGNS[resultData?.current_sign - 1]}.
 </div>
 </div>
 </div>
 ...
 <h2 className="font-serif text-3xl font-bold">Why use our <span className="text-[var(--gold)]">Zodiac Sign Finder</span>?</h2>
 <div className="prose prose-invert max-w-none text-gray-600 font-light text-sm leading-relaxed space-y-4">
 <p>
 In Vedic Astrology and numerology, precision is everything. Our free online Zodiac Sign provides you with highly accurate insights based on ancient mathematical algorithms combined with modern astronomical data.
 </p>
 <p>
