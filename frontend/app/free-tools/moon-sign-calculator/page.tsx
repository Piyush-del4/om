...
import { Moon, Search, ArrowRight, HelpCircle } from 'lucide-react';
...
const FAQS = [
  ...
  { q: 'How accurate is this Moon Sign Calculator?', a: 'Our calculator uses precise astronomical ephemeris data to generate highly accurate results based on Vedic Astrology (Jyotish).' },
  { q: 'Is this service completely free?', a: 'Yes, this tool is 100% free to use for unlimited calculations.' },
  { q: 'Do I need my exact birth time?', a: 'Yes, the Moon moves very quickly (changing signs every 2.5 days). Exact birth time ensures complete accuracy.' },
...
export default function MoonSignCalculatorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const moonData = Object.values(planetsObj).find((p: any) => p.name === 'Moon');
      
      if (data.success && moonData) {
        setResultData(moonData);
        setResult(true);
      } else {
      ...
      <div className="max-w-4xl mx-auto space-y-16 relative z-10">
        ...
        >
          <span className="text-[var(--gold)] text-xs uppercase tracking-widest font-semibold flex items-center justify-center gap-2">
            <Moon className="w-4 h-4" /> Free Utility
          </span>
          ...
          {!result ? (
            ...
            >
              <div className="w-16 h-16 bg-[var(--gold-50)] rounded-full flex items-center justify-center mx-auto border border-[var(--gold-200)]">
                <Moon className="w-8 h-8 text-[var(--gold)]" />
              </div>
              <div>
                <h3 className="text-2xl font-serif font-bold text-white mb-2">Report Generated!</h3>
                <p className="text-gray-400 text-sm mb-6">
                  Here is the precise Moon Sign (Rashi) for <span className="capitalize text-[var(--gold)]">{formData.name}</span>.
                </p>
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-8 text-center max-w-sm mx-auto">
                  ...
                  <div className="text-sm text-gray-300 font-light">
                    Your Moon is positioned at {resultData?.normDegree?.toFixed(2)}° in the sign of {ZODIAC_SIGNS[resultData?.current_sign - 1]?.split(' ')[0]}.
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6 pt-12 border-t border-neutral-800/60">
          <h2 className="font-serif text-3xl font-bold">Why use our <span className="text-[var(--gold)]">Moon Sign Calculator</span>?</h2>
          <div className="prose prose-invert max-w-none text-gray-300 font-light text-sm leading-relaxed space-y-4">
            <p>
              In Vedic Astrology and numerology, precision is everything. Our free online Moon Sign provides you with highly accurate insights based on ancient mathematical algorithms combined with modern astronomical data.
            </p>
            <p>
