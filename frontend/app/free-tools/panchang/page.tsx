...
import { Calendar, Search, ArrowRight, HelpCircle } from 'lucide-react';
...
const FAQS = [
  ...
  { q: "How accurate is this Today's Panchang?", a: 'Our calculator uses precise astronomical ephemeris data to generate highly accurate results.' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5001/api/v1/astrology/proxy', {
        ...
        body: JSON.stringify({
          endpoint: 'planets', // Replace with exact endpoint, e.g. 'advanced-panchang' or 'planets'
          data: {
            ...
            seconds: 0,
            latitude: 28.6139, // Default to New Delhi for now, requires geocoding API to resolve formData.location
            longitude: 77.2090,
            timezone: 5.5,
            config: {
      ...
      <div className="max-w-4xl mx-auto space-y-16 relative z-10">
        ...
        >
          <span className="text-[var(--gold)] text-xs uppercase tracking-widest font-semibold flex items-center justify-center gap-2">
            <Calendar className="w-4 h-4" /> Free Utility
          </span>
          <h1 className="font-serif text-4xl md:text-5xl font-bold">
            Today's  <span className="gold-gradient-text">Panchang</span>
          </h1>
          <p className="text-gray-400 text-sm md:text-base font-light max-w-2xl mx-auto">
            Check today's daily Panchang and auspicious timings. Enter your details below to calculate your personalized report instantly.
          </p>
        </motion.div>
          ...
          {!result ? (
            ...
            >
              ...
              <div>
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 text-left max-h-64 overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-6 pt-12 border-t border-neutral-800/60">
          <h2 className="font-serif text-3xl font-bold">Why use our <span className="text-[var(--gold)]">Today's Panchang</span>?</h2>
          <div className="prose prose-invert max-w-none text-gray-300 font-light text-sm leading-relaxed space-y-4">
            <p>
              In Vedic Astrology and numerology, precision is everything. Our free online Panchang provides you with highly accurate insights based on ancient mathematical algorithms combined with modern astronomical data.
            </p>
            <p>
