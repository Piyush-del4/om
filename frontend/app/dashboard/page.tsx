...
import { Calendar, Package, BookOpen, User as UserIcon, Clock, AlertCircle, Star } from 'lucide-react';
...
export default function UserDashboard() {
  ...
  const quickTiles = [
    { name: 'Premium Kundli', icon: <Star className="w-6 h-6 text-[var(--gold)]" />, desc: 'Generate your personalized Janam Kundli report', path: '/free-tools/kundli-generator', highlight: true },
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        <div className="border-b border-[var(--gold-200)] pb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            ...
            <p className="text-gray-400 text-sm font-light">Welcome to your personalized dashboard.</p>
          </div>
          <Link href="/#book">
            ...
            <Link key={tile.name} href={tile.path} className="group block h-full">
              <GoldCard className={`h-full transition-spring ${'highlight' in tile && tile.highlight ? 'border-[var(--gold)] bg-gradient-to-br from-[var(--gold-50)] to-black' : ''}`}>
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="font-serif text-lg font-bold text-white mb-1 group-hover:text-[var(--gold)] transition-colors">{tile.name}</h3>
                    <p className="text-gray-400 text-xs leading-relaxed font-light">{tile.desc}</p>
                  </div>
                </div>
              </GoldCard>
            </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <GoldCard theme="dark" className="transition-spring">
            <div className="space-y-4">
              <h3 className="font-serif text-xl font-bold text-[var(--gold)] flex items-center gap-2.5 pb-2 border-b border-neutral-800/60">
              ...
              <div className="space-y-3">
                {appointments && appointments.length > 0 ? (
                  appointments.slice(0, 3).map((app: any) => {
                    ...
                    return (
                      <div key={app._id} className="bg-neutral-900/40 border border-neutral-800/60 rounded-xl p-4 flex justify-between items-center text-sm transition-spring hover:border-[var(--gold-300)]">
                        <div className="space-y-1">
                          <p className="font-bold text-white text-sm">{app.typeName}</p>
              ...
              <div className="space-y-4">
                {enrolments && enrolments.length > 0 ? (
                  enrolments.map((enr: any) => (
                    <div key={enr._id} className="space-y-2.5 p-4 bg-neutral-900/40 border border-neutral-800/60 rounded-xl transition-spring hover:border-[var(--gold-300)]">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-bold text-white text-sm">{enr.batchId?.title}</span>
