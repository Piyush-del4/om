...
export default function AdminTeamPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-24 px-4 sm:px-6 lg:px-8 pt-24">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--gold-200)] pb-6">
          <div className="space-y-1">
            ...
            <p className="text-gray-400 text-xs font-light">Manage consultant profiles displayed on the About Us page.</p>
          </div>
          <div className="flex gap-3 flex-wrap">
            <button
              onClick={() => { setSeedMsg(''); seedMutation.mutate(); }}
              disabled={seedMutation.isPending}
              className="flex items-center gap-2 text-xs px-4 py-2.5 rounded-lg bg-neutral-800 border border-neutral-700 text-gray-300 hover:text-white hover:border-neutral-600 transition-colors"
            >
        ...
        {/* Edit / Create Form */}
        {editingId && (
          <GoldCard theme="dark" className="border border-[var(--gold-300)] p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Full Name *</label>
                <input
                  value={form.name}
              ...
              {/* Role */}
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Role / Title *</label>
                <input
                  value={form.role}
              ...
              {/* Initials */}
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Initials (max 3) *</label>
                <input
                  value={form.initials}
              ...
              {/* Experience Years */}
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Experience (Years)</label>
                <input
                  type="number"
                  min={0}
              ...
              {/* Image Fit */}
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Image Fit</label>
                <select
                  value={form.imageFit}
                  onChange={e => setForm({ ...form, imageFit: e.target.value as 'cover' | 'contain' })}
                  className="w-full bg-black/60 border border-neutral-700 focus:border-[var(--gold)] rounded-lg py-2.5 px-3 text-white text-xs outline-none appearance-none"
                >
              ...
              {/* Order */}
              <div className="space-y-1.5">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Display Order</label>
                <input
                  type="number"
                  value={form.order}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Specializations</label>
                ...
                {form.specializations.map((spec, idx) => (
                  <div key={idx} className="flex gap-2 items-center">
                    <input
                      ...
                      className="flex-1 bg-black/60 border border-neutral-700 focus:border-[var(--gold)] rounded-lg py-2 px-3 text-white text-xs outline-none"
                    />
            ...
            {/* Description */}
            <div className="space-y-1.5">
              <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400">Biography / Description</label>
              <textarea
                value={form.description}
                onChange={e => setForm({ ...form, description: e.target.value })}
                rows={5}
                placeholder="Write a detailed bio for this team member..."
                className="w-full bg-black/60 border border-neutral-700 focus:border-[var(--gold)] rounded-lg py-2.5 px-3 text-white text-xs outline-none resize-y leading-relaxed"
              />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="px-6 py-2.5 text-xs border border-neutral-700 rounded-lg text-gray-400 hover:text-white hover:border-neutral-500 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="space-y-4">
          {members.length === 0 ? (
            ...
            <GoldCard key={member._id} theme="dark" className="border border-neutral-800 hover:border-neutral-700 transition-all duration-300">
              <div className="flex items-start gap-4">
                {/* Avatar */}
                <div className="flex-shrink-0 w-14 h-14 rounded-xl overflow-hidden bg-neutral-900 border border-neutral-800">
                  {member.image ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {(member.specializations || []).slice(0, 4).map((s) => (
                      <span key={s} className="text-[9px] bg-neutral-900 border border-neutral-800 text-gray-400 px-2 py-0.5 rounded-full">
                        {s}
                      </span>
                    ))}
                  ...
                  {expandedId === member._id && (
                    <div className="mt-4 pt-4 border-t border-neutral-900 space-y-3">
                      <p className="text-gray-400 text-xs leading-relaxed line-clamp-4">{member.description}</p>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-neutral-900">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-gray-300 hover:text-[var(--gold)] hover:border-[var(--gold-200)] transition-colors"
                >
