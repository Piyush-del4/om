      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <GoldCard theme="dark" className="border border-[var(--gold-100)] p-6 space-y-4">
              ...
              <form onSubmit={handleSubmitType} className="space-y-4">
                <div className="space-y-1">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Name</label>
                  <input type="text" value={typeName} onChange={(e) => setTypeName(e.target.value)} required placeholder="e.g. Premium Kundali Reading" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Price (INR)</label>
                    <input type="number" value={priceInRupees} onChange={(e) => setPriceInRupees(e.target.value)} required placeholder="e.g. 1500" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Duration (Mins)</label>
                    <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} required placeholder="30" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Category</label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} required className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]">
                    <option value="Astrology" className="bg-neutral-900">Astrology</option>
                    <option value="Numerology" className="bg-neutral-900">Numerology</option>
                    <option value="Tarot Card" className="bg-neutral-900">Tarot Card</option>
                    <option value="Graphology" className="bg-neutral-900">Graphology</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Special Offer Title (Optional)</label>
                  <input type="text" value={specialOfferTitle} onChange={(e) => setSpecialOfferTitle(e.target.value)} placeholder="e.g. Festive Discount" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                </div>

                <div className="space-y-1">
                  <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Offer Price (INR) (Optional)</label>
                  <input type="number" value={offerPriceInRupees} onChange={(e) => setOfferPriceInRupees(e.target.value)} placeholder="e.g. 999" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                </div>
            ...
            <div className="space-y-3">
              <h4 className="font-serif text-sm font-bold text-white">Active Consultation Offerings</h4>
              {loadingTypes ? (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h5 className="font-bold text-white truncate">{type.name}</h5>
                            <span className="text-[9px] bg-neutral-800 text-[var(--gold)] border border-[var(--gold-100)] rounded-full px-1.5 py-0.5 flex-shrink-0">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {loadingBookings ? (
              <p className="text-gray-400 text-xs animate-pulse">Loading list...</p>
            ) : bookings && bookings.length > 0 ? (
              <div className="space-y-4">
                {bookings.map((booking: any) => {
                  const dateObj = new Date(booking.scheduledAt);
                  return (
                    <GoldCard key={booking._id} theme="dark" className="border border-neutral-800 p-4">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-white">{booking.typeName}</span>
                          ...
                          <p className="text-gray-400 text-xs">User: {booking.userId?.name || 'Unknown'} ({booking.userId?.email || 'N/A'}) · Phone: {booking.userId?.phone || 'N/A'}</p>
        ...
        <div className="border-t border-[var(--gold-100)]/30 pt-10 space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-serif text-xl font-bold text-white">Block Time Slots</h2>
              <p className="text-gray-400 text-xs mt-0.5">Prevent users from booking specific time windows. Blocked slots will not appear in the availability calendar.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2">
              <GoldCard theme="dark" className="border border-red-900/30 p-6 space-y-5">
                ...
                <form onSubmit={handleBlockSlot} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Start Date</label>
                      <input
                        type="date"
                        min={todayStr}
                        value={blockDate}
                        ...
                        required
                        className="w-full bg-black/60 border border-red-900/40 rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-red-700 [color-scheme:dark]"
                      />
                    </div>
                    <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Start Time (IST)</label>
                      <select
                        value={blockStartIdx}
                        onChange={(e) => {
                        ...
                        className="w-full bg-black/60 border border-red-900/40 rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-red-700"
                      >
                        {TIME_LABELS.slice(0, -1).map((t, i) => (
                          <option key={i} value={i} className="bg-neutral-900">{t.label}</option>
                        ))}
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">End Time (IST)</label>
                      <select
                        value={blockEndIdx}
                      ...
                      >
                        {endTimeOptions.map((t, relIdx) => {
                          ...
                          return (
                            <option key={absIdx} value={absIdx} className="bg-neutral-900">{t.label}</option>
                          );
                        })}
                      </select>
                    </div>
              ...
              {loadingBlocks ? (
                ...
                <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1 custom-scroll">
                  {blockedSlots.map((block: any) => (
                    <div
                      key={block._id}
                      className="flex items-center justify-between gap-3 px-4 py-3 bg-neutral-900 border border-red-900/20 rounded-xl group hover:border-red-900/40 transition-colors"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-white text-xs font-medium truncate">{block.label || 'Blocked'}</p>
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center space-y-3 border border-dashed border-neutral-800 rounded-2xl">
