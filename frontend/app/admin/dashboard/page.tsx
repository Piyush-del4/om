...
import { LayoutDashboard, Users, Calendar, ShoppingBag, GraduationCap, ArrowRight, TrendingUp } from 'lucide-react';
    ...
    <div className="min-h-screen bg-black text-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-10">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Hello, {user.name}. Here is an overview of the OM Astrology AMC portal status.</p>
        </div>

        {/* Metrics Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <GoldCard theme="dark" className="border border-neutral-800 p-6 flex items-center gap-4">
            <div className="p-3 bg-[var(--gold-10)] border border-[var(--gold-200)] text-[var(--gold)] rounded-xl">
              <GraduationCap className="w-6 h-6" />
            </div>
            <div>
              <p className="text-[10px] uppercase font-bold text-gray-400">Active Batches</p>
              <h3 className="text-2xl font-serif font-bold text-white mt-1">{activeBatchesCount}</h3>
            </div>
          </GoldCard>
          ...
          <GoldCard theme="dark" className="border border-neutral-800 p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-gray-400">Total Consultations</p>
              <h3 className="text-2xl font-serif font-bold text-white mt-1">{bookedAppointmentsCount}</h3>
            </div>
          </GoldCard>
          ...
          <GoldCard theme="dark" className="border border-neutral-800 p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-gray-400">Shop Orders</p>
              <h3 className="text-2xl font-serif font-bold text-white mt-1">{ordersCount}</h3>
            </div>
          </GoldCard>
          ...
          <GoldCard theme="dark" className="border border-neutral-800 p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-gray-400">Shop Revenue</p>
              <h3 className="text-2xl font-serif font-bold text-white mt-1">₹{totalRevenue.toLocaleString()}</h3>
            </div>
          </GoldCard>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {quickNav.map((item, idx) => (
              <div
                ...
                className="group cursor-pointer p-6 rounded-2xl bg-neutral-900/30 border border-neutral-800 hover:border-[var(--gold)] transition-all flex flex-col justify-between"
              >
                <div>
                  <div className={`p-2.5 rounded-lg bg-black/60 border border-neutral-800 inline-block mb-4 ${item.color}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  <h3 className="font-bold text-sm text-white group-hover:text-[var(--gold)] transition-colors">{item.title}</h3>
