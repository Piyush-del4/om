...
export default function EditBatchPage() {
  ...
  if (isLoading || loadingBatch || !user || user.role !== 'admin') {
    return <div className="min-h-screen bg-black flex items-center justify-center text-gray-400">Loading Batch Details...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        <button onClick={() => router.push('/admin/batches')} className="text-gray-400 hover:text-white transition-colors flex items-center gap-2 text-sm font-medium">
          <ArrowLeft className="w-4 h-4" /> Back to Batches list
        </button>

        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="font-serif text-3xl font-bold text-white">{batch?.title}</h1>
            <p className="text-gray-400 text-xs mt-1">Manage lectures, download worksheets, and check student progress.</p>
          </div>
        </div>
        ...
        <div className="pt-4">
          ...
          {activeTab === 'details' && (
            <div className="max-w-2xl">
              <GoldCard theme="dark" className="border border-[var(--gold-100)] p-6 space-y-4">
                <h3 className="font-serif text-lg font-bold text-white">Edit Batch Settings</h3>
                ...
                <form onSubmit={handleUpdateBatch} className="space-y-4">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Title</label>
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Price (INR)</label>
                      <input type="number" value={priceInRupees} onChange={(e) => setPriceInRupees(e.target.value)} required className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Access Code</label>
                      <input type="text" value={batchCode} onChange={(e) => setBatchCode(e.target.value)} className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Special Offer Title (Optional)</label>
                      <input type="text" value={specialOfferTitle} onChange={(e) => setSpecialOfferTitle(e.target.value)} placeholder="e.g. Diwali discount" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Offer Price (INR) (Optional)</label>
                      <input type="number" value={offerPriceInRupees} onChange={(e) => setOfferPriceInRupees(e.target.value)} placeholder="e.g. 3999" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Offer Expiry (Optional)</label>
                      <input type="datetime-local" value={offerExpiresAt} onChange={(e) => setOfferExpiresAt(e.target.value)} className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                    </div>
                  </div>

                  <div className="space-y-1">
                    <label className="block text-xs font-semibold uppercase tracking-wider text-gray-300">Cover Image *</label>
                    {coverUrl ? (
                      <div className="space-y-2">
                        <div className="relative border border-[var(--gold-100)] rounded-lg p-3 bg-black/40 flex items-center gap-3">
                          <img src={coverUrl} alt="Uploaded cover preview" className="w-16 h-12 object-cover rounded-lg border border-neutral-800" />
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] text-gray-400 truncate font-mono">{coverUrl}</p>
                      ...
                      <div className="relative border border-dashed border-[var(--gold-100)] hover:border-[var(--gold)] rounded-lg p-5 flex flex-col items-center justify-center bg-black/40 transition-colors cursor-pointer group">
                        <input
                          type="file"
                          accept="image/*"
                        ...
                        />
                        <Upload className="w-8 h-8 text-gray-400 group-hover:text-[var(--gold)] transition-colors mb-2" />
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2.5 px-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--gold)] text-xs" />
                  </div>
          ...
          {activeTab === 'lectures' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1">
                <GoldCard theme="dark" className="border border-[var(--gold-100)] p-5 space-y-4">
                  <h3 className="font-serif text-sm font-bold text-white flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add Lecture</h3>
                  ...
                  <form onSubmit={handleAddLecture} className="space-y-3">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Lecture Title</label>
                      <input type="text" value={lectureTitle} onChange={(e) => setLectureTitle(e.target.value)} required placeholder="Lecture 1: Introduction" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">YouTube Video ID</label>
                      <input type="text" value={youtubeVideoId} onChange={(e) => setYoutubeVideoId(e.target.value)} required placeholder="e.g. dQw4w9WgXcQ" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Sort Order</label>
                      <input type="number" value={lectureOrder} onChange={(e) => setLectureOrder(e.target.value)} required placeholder="0" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <h3 className="font-serif text-sm font-bold text-white">Published Lectures</h3>
                {loadingLectures ? (
                  <p className="text-gray-400 text-xs animate-pulse">Loading lectures list...</p>
                ) : lectures && lectures.length > 0 ? (
                  <div className="space-y-2">
                    {lectures.map((lec: any) => (
                      <div key={lec._id} className="p-4 bg-neutral-900 border border-neutral-800 rounded-xl flex items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h5 className="font-bold text-white">{lec.title}</h5>
                            <p className="text-[10px] text-gray-400">Order: {lec.order} · YouTube ID: <span className="font-mono">{lec.youtubeVideoId}</span></p>
                          </div>
                        </div>
          ...
          {activeTab === 'pdfs' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1">
                <GoldCard theme="dark" className="border border-[var(--gold-100)] p-5 space-y-4">
                  <h3 className="font-serif text-sm font-bold text-white flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add Note</h3>
                  ...
                  <form onSubmit={handleAddPdf} className="space-y-3">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Document Title</label>
                      <input type="text" value={pdfTitle} onChange={(e) => setPdfTitle(e.target.value)} required placeholder="e.g. Numerology Reference Chart" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">PDF Document URL</label>
                      <input type="url" value={pdfUrl} onChange={(e) => setPdfUrl(e.target.value)} required placeholder="https://example.com/sheet.pdf" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Cloudinary Public ID</label>
                      <input type="text" value={pdfPublicId} onChange={(e) => setPdfPublicId(e.target.value)} placeholder="note-pdf" className="w-full bg-black/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-300">Associated Lecture (Optional)</label>
                      <select value={pdfLectureId} onChange={(e) => setPdfLectureId(e.target.value)} className="w-full bg-neutral-900 border border-neutral-800 rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]">
              ...
              {/* PDFs List */}
              <div className="lg:col-span-2 space-y-4">
                <h3 className="font-serif text-sm font-bold text-white">Notes</h3>
                {loadingPdfs ? (
                  <p className="text-gray-400 text-xs animate-pulse">Loading files list...</p>
                ) : pdfs && pdfs.length > 0 ? (
                  <div className="space-y-2">
                    {pdfs.map((pdf: any) => (
                      <div key={pdf._id} className="p-4 bg-neutral-900 border border-neutral-800 rounded-xl flex items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h5 className="font-bold text-white">{pdf.title}</h5>
                            <p className="text-[10px] text-gray-400">File: <a href={pdf.url} target="_blank" rel="noopener noreferrer" className="text-[var(--gold)] hover:underline truncate inline-block max-w-[200px]">{pdf.url}</a></p>
                          </div>
                        </div>
          ...
          {activeTab === 'progress' && (
            <div className="space-y-4">
              <h3 className="font-serif text-sm font-bold text-white flex items-center gap-2"><Users className="w-4.5 h-4.5 text-[var(--gold)]" /> Enrolled Students & Progression Logs</h3>
              {loadingEnrolments ? (
                <p className="text-gray-400 text-xs animate-pulse">Loading enrolments stats...</p>
                    ...
                    <tbody className="divide-y divide-neutral-800/60 text-gray-300">
                      {enrolments.map((enr: any, index: number) => (
                        <tr key={index} className="hover:bg-neutral-900/20">
                          <td className="py-3.5 px-4 font-medium text-white">{enr.userName}</td>
                          <td className="py-3.5 px-4 font-mono">{enr.email}</td>
                          <td className="py-3.5 px-4 font-mono">{enr.phone || 'N/A'}</td>
                          <td className="py-3.5 px-4"><span className="uppercase text-[9px] bg-neutral-900 border border-neutral-800 px-2 py-0.5 rounded-full">{enr.method}</span></td>
                          <td className="py-3.5 px-4">{new Date(enr.createdAt).toLocaleDateString()}</td>
                          <td className="py-3.5 px-4 text-right">
                            <span className="font-bold text-white">{enr.watchedCount} / {enr.totalLectures}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {loadingAnnouncements ? (
                  <p className="text-gray-400 text-xs animate-pulse">Loading announcements list...</p>
                ) : announcements && announcements.length > 0 ? (
                  <div className="space-y-3">
                    {announcements.map((ann: any, idx: number) => (
                      <div key={idx} className="p-4 bg-neutral-900 border border-neutral-800 rounded-xl space-y-1 text-xs">
                        <div className="flex justify-between items-center text-[10px] text-gray-500 font-mono">
                          <span>Sent By Admin</span>
                          <span>{new Date(ann.createdAt).toLocaleString()}</span>
                        </div>
                        <p className="text-gray-200 leading-relaxed whitespace-pre-wrap">{ann.message}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-gray-500 text-xs py-8 text-center bg-neutral-950/10 border border-dashed border-neutral-800 rounded-xl">
