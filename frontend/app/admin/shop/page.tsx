...
export default function AdminShopPage() {
 ...
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Product Title</label>
 <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g. Shaligram Stone" className="w-full bg-white/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
 </div>
 
 <div className="space-y-1">
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Price (INR)</label>
 <input type="number" value={priceInRupees} onChange={(e) => setPriceInRupees(e.target.value)} required placeholder="e.g. 500" className="w-full bg-white/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
 </div>
 
 <div className="space-y-1">
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Special Offer Title (Optional)</label>
 <input type="text" value={specialOfferTitle} onChange={(e) => setSpecialOfferTitle(e.target.value)} placeholder="e.g. Diwali Sale" className="w-full bg-white/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
 </div>
 
 <div className="space-y-1">
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Offer Price (INR) (Optional)</label>
 <input type="number" value={offerPriceInRupees} onChange={(e) => setOfferPriceInRupees(e.target.value)} placeholder="e.g. 350" className="w-full bg-white/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
 </div>
 ...
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Product Images (Optional)</label>
 
 {images.length > 0 && (
 <div className="grid grid-cols-4 gap-2 mb-3">
 {images.map((url, idx) => (
 <div key={idx} className="relative group border border-gray-200 rounded-lg overflow-hidden h-12 w-full bg-gray-100 text-center">
 ...
 />
 <Upload className="w-6 h-6 text-gray-600 group-hover:text-[var(--gold)] transition-colors mb-2" />
 <span className="text-[10px] text-gray-600 group-hover:text-gray-900 transition-colors text-center">
 ...
 />
 <label htmlFor="inStock" className="text-[10px] font-semibold uppercase tracking-wider text-gray-600 cursor-pointer">
 In Stock
 </label>
 </div>
 <div className="space-y-1">
 <label className="block text-[10px] font-semibold uppercase tracking-wider text-gray-600">Pieces Left <span className="text-gray-500 normal-case">(optional)</span></label>
 <input
 type="number"
 min="0"
 value={stockCount}
 ...
 <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Product description..." rows={3} className="w-full bg-white/60 border border-[var(--gold-100)] rounded-lg py-2 px-3 text-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--gold)]" />
 </div>
 ...
 <h4 className="font-serif text-sm font-bold text-gray-900">Inventory Items</h4>
 {loadingItems ? (
 ...
 {loadingOrders ? (
 <p className="text-gray-600 text-xs animate-pulse">Loading orders...</p>
 ) : orders && orders.length > 0 ? (
 <div className="overflow-x-auto rounded-xl border border-gray-200 bg-gray-100/20">
 ...
 <tr key={order._id} className="hover:bg-gray-100/20">
 <td className="py-3.5 px-4 font-mono text-[10px] text-gray-600">
 <div className="text-gray-900 font-bold">#{order._id.substring(order._id.length - 8)}</div>
 ...
 {item.title || 'Unknown Product'} <strong className="text-gray-900">x{item.quantity}</strong>
 </div>
 ))}
 </div>
 </td>
 ...
 className="bg-white border border-gray-200 text-[10px] rounded-lg py-1 px-2 text-gray-900 focus:outline-none focus:ring-1 focus:ring-[var(--gold)] cursor-pointer"
 >
 ...
 <p className="text-gray-500 text-xs py-8 text-center bg-gray-50/10 border border-dashed border-gray-200 rounded-xl">No orders logs recorded.</p>
 )}
 </>
 ) : (
 ...
 <div className="p-4 bg-gray-100/40 border border-gray-200 rounded-xl space-y-1">
 ...
 <td className="py-3 px-4 text-right text-gray-900 font-bold">{row.unitsSold} units</td>
