...
function SingleHouseCard({ info }: { info: any }) {
  return (
    <div className="pdf-page-break-avoid bg-white dark:bg-neutral-800 border border-amber-300 dark:border-neutral-700 rounded-xl p-5 space-y-4 shadow-sm">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200 dark:border-neutral-700 pb-3 gap-2">
        <div>
        ...
        <span className="px-3 py-1 bg-amber-200 dark:bg-amber-900/50 text-amber-950 dark:text-amber-300 rounded-full text-xs font-bold">
          Status: {info.strength}
        </span>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs md:text-sm">
        <div className="bg-amber-50 dark:bg-neutral-900 p-3 rounded-lg border border-amber-200 dark:border-neutral-700">
          <span className="text-gray-500 dark:text-gray-400 block text-xs">Zodiac Sign</span>
          <span className="font-bold text-amber-950 dark:text-white text-base">{info.signName} (Sign #{info.signNum})</span>
        </div>

        <div className="bg-amber-50 dark:bg-neutral-900 p-3 rounded-lg border border-amber-200 dark:border-neutral-700">
          <span className="text-gray-500 dark:text-gray-400 block text-xs">House Lord</span>
          <span className="font-bold text-amber-950 dark:text-white text-base">{info.lord}</span>
        </div>

        <div className="bg-amber-50 dark:bg-neutral-900 p-3 rounded-lg border border-amber-200 dark:border-neutral-700">
          <span className="text-gray-500 dark:text-gray-400 block text-xs">Occupying Planets</span>
          <span className="font-bold text-amber-950 dark:text-white text-base">
      ...
      <div className="space-y-3 text-xs md:text-sm pt-2">
        <div>
          <h5 className="font-bold text-amber-900 dark:text-amber-400">Interpretation</h5>
          <p className="text-gray-700 dark:text-gray-300 mt-1">{info.interpretation}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg">
            <h6 className="font-bold text-emerald-900 dark:text-emerald-300">Positive Effects</h6>
            <p className="text-emerald-950 dark:text-emerald-200 text-xs mt-1">{info.positiveEffects}</p>
          </div>

          <div className="p-3 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 rounded-lg">
            <h6 className="font-bold text-rose-900 dark:text-rose-300">Challenges</h6>
            <p className="text-rose-950 dark:text-rose-200 text-xs mt-1">{info.challenges}</p>
          </div>
        </div>

        <div className="p-3 bg-amber-100/70 dark:bg-neutral-900 border border-amber-300 dark:border-neutral-700 rounded-lg">
          <h6 className="font-bold text-amber-950 dark:text-amber-300">Traditional Remedies</h6>
          <p className="text-amber-900 dark:text-gray-300 text-xs mt-1">{info.remedies}</p>
        </div>
      </div>
    </div>
  );
