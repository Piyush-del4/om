  ...
  return (
    <div className="bg-amber-50/60 dark:bg-neutral-900 border-2 border-amber-800/30 rounded-2xl p-6 w-full max-w-5xl mx-auto my-6 shadow-lg space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {items.map((item) => (
          <div key={item.title} className="p-4 bg-white dark:bg-neutral-800 border border-amber-300 dark:border-neutral-700 rounded-xl space-y-2 flex flex-col justify-between shadow-sm">
            <div className="flex items-center gap-2 border-b border-gray-100 dark:border-neutral-700 pb-2">
              <span className="text-2xl">{item.icon}</span>
              <span className="font-bold text-amber-950 dark:text-amber-300 text-xs">{item.title}</span>
            </div>

            <div>
              <span className="font-bold text-amber-900 dark:text-white text-base block">{item.val}</span>
