      ...
      <div className="flex justify-center gap-3">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all ${
            filter === 'all' ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 dark:bg-neutral-800 text-amber-950 dark:text-gray-300'
          }`}
        >
          All Alignments
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all ${
            filter === 'aspects' ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 dark:bg-neutral-800 text-amber-950 dark:text-gray-300'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all ${
            filter === 'conjunctions' ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 dark:bg-neutral-800 text-amber-950 dark:text-gray-300'
          }`}
        >
          ...
          {conjunctions.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {conjunctions.map((conj, idx) => (
                <div key={idx} className="p-4 bg-amber-50 dark:bg-neutral-900 border border-amber-200 dark:border-neutral-700 rounded-xl space-y-2">
                  <div className="flex justify-between items-center border-b pb-2 border-amber-200 dark:border-neutral-700">
                    <span className="font-bold text-amber-950 dark:text-amber-300 text-sm">
                      {conj.planets.join(' + ')}
                    </span>
                    <span className="px-2 py-0.5 bg-amber-200 dark:bg-amber-900 text-amber-950 dark:text-amber-300 rounded text-xs font-bold">
                      House {conj.houseNumber} ({conj.zodiacSign})
                    </span>
                  </div>

                  <p className="text-xs text-gray-700 dark:text-gray-300">{conj.effects}</p>

                  <div className="text-[11px] space-y-1 pt-1">
                    <div className="text-emerald-800 dark:text-emerald-300 font-semibold">
                      ✓ Positive: {conj.positivePoints.join(' ')}
                    </div>
                    <div className="text-rose-800 dark:text-rose-300 font-semibold">
          ...
          <table className="w-full text-left border-collapse text-[11px] sm:text-xs md:text-sm">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-neutral-700 font-medium">
              {aspects.slice(0, 15).map((asp, idx) => (
                <tr key={idx} className="hover:bg-amber-50 dark:hover:bg-neutral-700/40">
                  <td className="px-2 py-2 sm:p-2.5 font-bold text-amber-950 dark:text-white">{asp.aspectingPlanet}</td>
                  ...
                  <td className="px-2 py-2 sm:p-2.5 text-gray-700 dark:text-gray-300 text-[11px] sm:text-xs">{asp.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
