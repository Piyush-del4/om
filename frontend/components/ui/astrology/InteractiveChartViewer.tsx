...
export function InteractiveChartViewer({ data, chartTitle = 'Lagna Chart (D-1)' }: Props) {
  ...
  return (
    <div className="bg-amber-50/60 dark:bg-neutral-900 border-2 border-amber-800/30 rounded-2xl p-6 w-full max-w-5xl mx-auto my-6 shadow-lg space-y-6">
      ...
      <div className="w-full bg-white dark:bg-neutral-800 border border-amber-300 dark:border-neutral-700 rounded-xl shadow-inner p-2">
            ...
            {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (
              <button
                ...
                className={`px-2.5 py-1 rounded text-xs font-bold transition ${
                  selectedHouseDrawer === h ? 'bg-amber-600 text-white' : 'bg-amber-100 dark:bg-neutral-700 text-amber-950 dark:text-gray-300 hover:bg-amber-200'
                }`}
              >
                H{h}
              </button>
      ...
      {selectedHouseDrawer && (
        <div className="bg-amber-100/70 dark:bg-neutral-800 border-2 border-amber-500 rounded-xl p-5 space-y-3 relative shadow-md">
          <button
            onClick={() => setSelectedHouseDrawer(null)}
            className="absolute top-3 right-3 text-amber-950 dark:text-white font-bold hover:text-rose-600 text-sm"
          >
            ✕ Close
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div className="p-2.5 bg-white dark:bg-neutral-900 rounded border">
              <span className="text-gray-500 block">Zodiac Sign</span>
              <span className="font-bold text-amber-950 dark:text-white text-sm">{selectedSignName}</span>
            </div>

            <div className="p-2.5 bg-white dark:bg-neutral-900 rounded border">
              <span className="text-gray-500 block">House Lord</span>
              <span className="font-bold text-amber-950 dark:text-white text-sm">{selectedLord}</span>
            </div>

            <div className="p-2.5 bg-white dark:bg-neutral-900 rounded border">
              <span className="text-gray-500 block">Occupants</span>
              <span className="font-bold text-amber-950 dark:text-white text-sm">
                {selectedOccupants.length > 0 ? selectedOccupants.join(', ') : 'None'}
              </span>
            </div>
          </div>

          <p className="text-xs md:text-sm text-gray-800 dark:text-gray-200 pt-1">
      ...
      {showCompareModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-neutral-900 border-2 border-amber-500 rounded-2xl p-6 max-w-3xl w-full space-y-4 shadow-2xl relative max-h-[85vh] overflow-y-auto">
            <button
              onClick={() => setShowCompareModal(false)}
              className="absolute top-4 right-4 text-amber-950 dark:text-white font-bold hover:text-rose-600 text-base"
            >
              ✕ Close
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-amber-50 dark:bg-neutral-800 rounded-xl border border-amber-300">
                <h5 className="font-bold text-amber-950 dark:text-amber-300 text-sm mb-2">Primary Chart (Natal)</h5>
                <NorthIndianChart data={data} title="Natal Kundli" />
              </div>

              <div className="p-4 bg-blue-50 dark:bg-neutral-800 rounded-xl border border-blue-300">
                <h5 className="font-bold text-blue-950 dark:text-blue-300 text-sm mb-2">Partner / Transit Chart</h5>
