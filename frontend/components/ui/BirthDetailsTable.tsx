...
export function BirthDetailsTable({
  ...
  return (
    <div className="w-full max-w-full mx-auto my-4 border-2 border-amber-800/40 rounded-xl overflow-hidden shadow-md bg-white dark:bg-neutral-900 text-sm sm:text-base">
      {/* Header with Name */}
      ...
      {/* Details Table */}
      <div className="divide-y divide-amber-800/20 dark:divide-neutral-800 font-medium">
        {rows.map((r, i) => (
          <div key={i} className="grid grid-cols-5 text-neutral-900 dark:text-gray-200 hover:bg-amber-100/50 dark:hover:bg-neutral-800/50 transition-colors">
            <div className="col-span-2 p-3 font-bold text-amber-950 dark:text-amber-200 border-r border-amber-800/20 dark:border-neutral-800">
              {r.label}
            </div>
            <div className="col-span-3 p-3 font-semibold text-neutral-800 dark:text-gray-200">
              {r.value}
            </div>
          </div>
        ))}
