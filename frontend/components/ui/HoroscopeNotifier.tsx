    ...
    const fetchAndShowHoroscope = async () => {
      ...
      try {
        ...
        if (response.data?.success && response.data?.data) {
          ...
          const rawSignHoroscope = horoscopeDoc[signLower] || horoscopeDoc.data?.[signLower];

          if (rawSignHoroscope) {
            const horoscopeText = typeof rawSignHoroscope === 'string' 
              ? rawSignHoroscope 
              : (rawSignHoroscope.daily?.career || rawSignHoroscope.daily?.married_life || rawSignHoroscope.daily?.family || '');
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3 flex-1">
                          ...
                          <p className="mt-1 text-xs text-gray-600 line-clamp-2 leading-relaxed">
                            {horoscopeText.split('.').slice(0, 2).join('.')}...
                          </p>
                        </div>
                      </div>
                    </div>
