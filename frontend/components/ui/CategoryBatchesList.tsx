...
export function CategoryBatchesList({ category }: CategoryBatchesListProps) {
    ...
    return (
      <div className="w-full max-w-5xl mx-auto py-8">
        <div className="text-center space-y-1 mb-8">
          <div className="w-48 h-5 bg-neutral-900/60 rounded animate-shimmer mx-auto mb-2" />
          <div className="w-64 h-3 bg-neutral-900/60 rounded animate-shimmer mx-auto" />
        </div>
  ...
  if (activeBatches.length === 0) {
    return (
      <div className="text-center py-10 border border-dashed border-neutral-800 rounded-2xl text-gray-500 text-xs">
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto w-full">
      <div className="text-center space-y-1 mb-8">
        ...
        <p className="text-gray-400 text-xs">Explore batches and register to unlock Lectures and Notes.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {activeBatches.map((batch: any) => {
          ...
          return (
            <GoldCard key={batch._id} theme="dark" className="border border-[var(--gold-100)] flex flex-col justify-between overflow-hidden p-6">
              <Link href={`/batches/${batch._id}`} className="block flex-grow cursor-pointer">
                {batch.coverImage?.url && (
                  <div className="w-full h-44 bg-neutral-900 overflow-hidden rounded-t-xl -mt-6 -mx-6 mb-4" style={{ width: 'calc(100% + 3rem)' }}>
