...
export function SignatureAnalyzerWidget() {
  ...
  const drawGrid = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
    ...
    const gridColor = isDark ? 'rgba(204, 143, 51, 0.07)' : 'rgba(160, 110, 34, 0.06)';
    const centerColor = isDark ? 'rgba(204, 143, 51, 0.18)' : 'rgba(160, 110, 34, 0.15)';
  ...
  return (
    <div className="w-full space-y-6 relative z-20">
      <GoldCard theme="dark" className="border border-[var(--gold-300)] p-6 flex flex-col space-y-4 shadow-[0_4px_30px_rgba(204,143,51,0.06)] relative">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={handleClear}
            className="text-gray-400 hover:text-red-400 p-1 rounded transition-colors duration-300 flex items-center gap-1 text-[10px] uppercase tracking-wider font-semibold border border-neutral-800 px-2.5 py-1"
          >
        ...
        <div className="relative w-full h-48 bg-neutral-950/70 border border-[var(--gold-200)] rounded-2xl overflow-hidden cursor-crosshair">
          <canvas
            ref={canvasRef}
        ...
        <div className="space-y-3 pt-2">
          <h4 className="font-serif text-xs font-bold text-white uppercase tracking-wider border-b border-neutral-800 pb-1.5 flex items-center gap-1.5">
          ...
          {isAnalyzed && metrics.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {metrics.map((m) => (
                <div key={m.label} className="p-3 bg-neutral-900/50 border border-[var(--gold-100)] rounded-xl space-y-1 relative overflow-hidden group hover:border-[var(--gold-300)] transition-colors duration-300">
                  ...
                  <p className="text-gray-400 text-[9px] md:text-[10px] leading-relaxed font-light">
                    {m.desc}
                  </p>
                </div>
              ))}
